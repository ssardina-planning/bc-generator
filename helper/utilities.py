...
def asynchronous_product(graph1, graph2):
    ...
    for u, v in product(graph1, graph2):
        ...
        l = _dict_product(graph1.node[u], graph2.node[v])
        GH.add_node(n, **l) # converts dictoinary l into kwargs (https://docs.python.org/3/tutorial/controlflow.html#unpacking-argument-lists)

    for u, v, k, d in graph1.edges(data=True, keys=True):
        for x in graph2:
            # print (u,x),(v,x),k,d
    ...
    for x in graph1:
        for u, v, k, d in graph2.edges(data=True, keys=True):
            # print (x,u),(x,v),k,d
...
def synchronous_product(graph1, graph2):
    ...
    for u1, v1, d1 in graph1.edges(data=True):
        for u2, v2, d2 in graph2.edges(data=True):
            if (d1 == d2):
...
def get_single_determinized(graph):
    ...
    # first collect all nd edges into nd_edges_grps
    # nd_edges_grps is a set of sets of non-det edges for the same node. So it contains groups of nd-edges
    nd_nodes = get_ndnodes(graph)
    nd_edges_grps = []
    for node in nd_nodes:
        edges = graph.edges([node], keys=True, data=True)
        labels = get_labels(edges)
        for label in labels:
            nd_edges = get_edges_with_label(label, edges)
            if len(nd_edges) > 1:
                nd_edges_grps.append(nd_edges)

    first_combination = []
    '''
    TODO: old code was get_startnode(graph.nodes()[0]) because graph.nodes() returned a list. Is 
    Now, graph.nodes gives a dictionary and hence there is no order. We need to conver that into a list
    But, did the 0 before meant the real start node of a graph?
    Check https://networkx.github.io/documentation/stable/release/migration_guide_from_1.x_to_2.0.html
    '''
    startnode = get_startnode(list(graph.nodes)[0])
    for nd_edges in nd_edges_grps:
        added = False
        for edge in nd_edges:
            if edge[1] == startnode and not added:
                ...
                added = True
        if not added:
            index = random.randrange(0, len(nd_edges), 1)
            first_combination.append(nd_edges[index])
    tmpgraph = graph.copy()

    for g in nd_edges_grps:
        for e in g:
